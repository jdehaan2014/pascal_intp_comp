...
type
  ...
  TExpressionParser = class(TStatementParser)
    public
      ...
      constructor Create(Parent: TPascalParserTD);
      function Parse(Token: TToken): ICodeNode;
      class procedure InitializeExprMaps; static;
      class procedure FinalizeExprMaps; static;
    private
      const
  ...
  TCaseStatementParser = class(TStatementParser)
    private
      type
        TConstantSet = specialize TFPGObjectList<TObject>;
      const
        //sync set for CASE
...
begin
  ...
  case TPascalTokenTyp.toTyp(Token.getTyp) of
    ...
    ttCase:
      ...
      StatementNode := TICodeFactory.CreateICodeNode(ctNoOp);
  end;
...
function TAssignmentStatementParser.ParseFunctionNameAssignment(Token: TToken)
  : ICodeNode;
begin
...
class procedure TExpressionParser.InitializeExprMaps;
begin
...
class procedure TExpressionParser.FinalizeExprMaps;
begin
  RelOpsMap.Free;
  AddOpsMap.Free;
  MulOpsMap.Free;
...
begin
  ...
  //for some reason the type is not copied, so copy Node and redo setTypeSpec
...
function TDeclarationsParser.Parse(Token: TToken; ParentID: ISymTabEntry)
  : ISymTabEntry;
var
...
begin
  ...
  if Token.getTyp = ttConst then begin
    ...
    ConstantDefinitionsParser.Parse(Token, Nil);
  end;
...
begin
  if Value is TInteger then
    ...
    if TString(Value).Length = 1 then
      ...
      constantType := TTypeFactory.CreateStringType(TString(Value));
  end;
  Result := constantType;
end;
...
function TConstantDefinitionsParser.Parse(Token: TToken; ParentID: ISymTabEntry)
  : ISymTabEntry;
var
  Name: string;
...
function TTypeDefinitionsParser.Parse(Token: TToken; ParentID: ISymTabEntry)
  : ISymTabEntry;
var
  Name: string;
  TypeID: ISymTabEntry;
...
begin
  ...
  if Form = tfSubrange then begin
    ...
    if (minValue <> Nil) and (maxValue <> Nil) then
      Count := maxValue.intValue - minValue.intValue + 1;
  end
...
begin
  if Token.getTyp = ttIdentifier then begin
    ...
    if ID = Nil then begin
      ...
      Slot := ID.getSymTab.nextSlotNumber;
      ID.setAttribute(skSlot, TInteger.Create(Slot));
    end
    else
      ErrorHandler.Flag(Token, ecIDENTIFIER_REDEFINED, Self);
...
begin
  ...
  //Formal parameters and functions must have typed names
  if (Definition <> defVariable) and (Definition <> defField) and
     (Typ <> Nil) and (Typ.getIdentifier = Nil) then
     ErrorHandler.Flag(Token, ecINVALID_TYPE, Self);
...
function TVariableDeclarationsParser.Parse(Token: TToken; ParentID: ISymTabEntry)
  : ISymTabEntry;
var
  TokenTyp: ITokenTyp;
begin
...
begin
  ...
  if not isFunctionTarget then begin
...
begin
  ...
  if (TokenTyp = ttIdentifier) and (VariableForm = tfRecord) then begin
    SymTab := VariableType.getAttribute(tkRecordSymTab) as ISymTab;
...
begin
  ...
  if RoutineDefn = defProgram then begin
...
begin
  ...
  while Token.getTyp <> ttRightParen do begin
    ...
    //                a scalar, boolean, or subrange of integer.
      ...
      Form := Typ.getForm;
      if not ((TICodeNodeTypeImpl.toTyp(ActualNode.getType) = ctVariable) and
...
initialization
  TExpressionParser.InitializeExprMaps;
finalization
  TExpressionParser.FinalizeExprMaps;
