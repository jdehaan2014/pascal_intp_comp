...
type
  ...
  ISymTab = interface(IAnyInterface)
    ...
    function nextSlotNumber: Integer; //return the next local variables array slot no
    function maxSlotNumber: Integer;  //return max local var array slot no
  end;
  ...
  //the interface for the routine code of standard or declared proc or func
  IRoutineCode = interface(IAnyInterface)
    ['IRoutineCode']
  end;
