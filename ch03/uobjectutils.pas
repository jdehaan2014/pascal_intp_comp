...
uses
  Classes, SysUtils, uTypeHelpers, AnyObject;
...
{function TDynArray.Equals(Index: integer; const Item: T): boolean;
var
   ti: PTypeInfo;
begin
  ti := TypeInfo(T);
  if ti = TypeInfo(TString) then
    Result := (FItems[Index] as TString).Equals(Item as TString)
  else f ti = TypeInfo(TBoolean) then
    Result := (FItems[Index] as TBoolean).Equals(Item as TBoolean)
  else

  if ti^.Kind = tkClass then begin
    case Uppercase(ti^.Name) of
      'TSTRING'  : Result := TString(FItems[Index]).Equals(TString(Item));
      'TBOOLEAN' : Result := TBoolean(FItems[Index]).Equals(TBoolean(Item));
      'TINTEGER' : Result := TInteger(FItems[Index]).Equals(TInteger(Item));
      'TFLOAT'   : Result := TFloat(FItems[Index]).Equals(TFloat(Item));
    end;
    //Result := TClass(FItems[Index]).Equals(TObject(Item))
...
{function TDynArray.IndexOf(const Item: T): integer;
var
  i: Integer;
begin
  Result := -1;
  for i := 0 to FCount-1 do
    if Equals(i, Item) then
      Exit(i);
end;   }
