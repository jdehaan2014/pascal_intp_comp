...
uses
  ...
  uPascalTokentyp, uTypeHelpers, uPascalToken, uPascalErrorCode, uObjectUtils;
...
begin
  ...
  else if CurrChar.isLetter then                          //keyword or ident
    Token := TPascalWordToken.Create(Source)
  else if CurrChar.isDigit then                           //number
    Token := TPascalNumberToken.Create(Source)
  else if CurrChar.isSingleQuote then                     //string
    Token := TPascalStringToken.Create(Source)
  else if TPascalTokenTyp.isSpecialSymbol(CurrChar) then  //operator or context
