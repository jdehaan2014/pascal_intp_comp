...
type
  ...
  TCallMsg = class(TMessageBody)
    SourceLineNumber: integer;
    CalledRoutineName: string;
    constructor Create(ASourceLineNumber: integer; ACalledRoutineName: string);
  end;

  { TReturnMsg }

  TReturnMsg = class(TMessageBody)
    SourceLineNumber: integer;
    ReturnedRoutineName: string;
    constructor Create(ASourceLineNumber: integer; AReturnedRoutineName: string);
  end;
...
begin
  Listeners.Free;
  if Assigned(Message) then Message.Free;
  Inherited;
end;
...
destructor TAssignMsg.Destroy;
begin
  if Assigned(ExpressionValue) then ExpressionValue.Free;
  inherited;
end;

{ TFetchMsg }

constructor TFetchMsg.Create(ASourceLineNumber: integer; ATargetVariableName: string; AExpressionValue: TObject);
begin
  SourceLineNumber := ASourceLineNumber;
  TargetVariableName := ATargetVariableName;
  ExpressionValue := AExpressionValue;
...
constructor TCallMsg.Create(ASourceLineNumber: integer; ACalledRoutineName: string);
begin
  SourceLineNumber := ASourceLineNumber;
  CalledRoutineName := ACalledRoutineName;
end;

{ TReturnMsg }

constructor TReturnMsg.Create(ASourceLineNumber: integer; AReturnedRoutineName: string);
begin
  SourceLineNumber := ASourceLineNumber;
  ReturnedRoutineName := AReturnedRoutineName;
end;
