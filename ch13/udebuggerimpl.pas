...
begin
  ...
  if Form = tfSubrange then begin
    minValue := TInteger(Typ.getAttribute(tkSubRangeMinValue));
    maxValue := TInteger(Typ.getAttribute(tkSubRangeMaxValue));
  end
  else if Form = tfEnumeration then begin
    Constants := TSymTabEntries(Typ.getAttribute(tkEnumerationConstants));
