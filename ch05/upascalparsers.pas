...
type
  ...
  //TOpsMap = specialize TFPGMap<TPascalTokenTyp.Values,
    //                           TICodeNodeTypeImpl.TCodeNodeType>;
...
begin
  StatementNode := Nil;
  case TPascalTokenTyp(Token.getTyp.getObject).Value of
    ttBegin:
      begin
    ...
    else StatementNode := TICodeFactory.CreateICodeNode(ctNoOp);
  end;
...
var
  AssignNode, VariableNode, ExprNode: ICodeNode;
  TargetName: string;
...
begin
  ...
  if RelOps.Contains(TokenTyp) then begin  // see typehelper TPascalTokenTypSet
    ...
    NodeType := RelOpsMap.KeyData[TPascalTokenTyp(TokenTyp.getObject).Value];
...
begin
  ...
  while AddOps.Contains(TokenTyp) do begin
    //Create a new operator node and adopt the current tree as its first child.
    //NodeType := AddOpsMap.KeyData[TPascalTokenTyp(TokenTyp.getObject).Value];
    NodeType := AddOpsMap.KeyData[TPascalTokenTyp(TokenTyp.getObject).Value];
    OpNode := TICodeFactory.CreateICodeNode(NodeType);
    OpNode.addChild(RootNode);
    Token := NextToken;  // consume the operator
...
begin
  ...
  while MulOps.Contains(TokenTyp) do begin
    //Create a new operator node and adopt the current tree as its first child.
    NodeType := MulOpsMap.KeyData[TPascalTokenTyp(TokenTyp.getObject).Value];
    OpNode := TICodeFactory.CreateICodeNode(NodeType);
    OpNode.addChild(RootNode);
    Token := NextToken;  // consume the operator
