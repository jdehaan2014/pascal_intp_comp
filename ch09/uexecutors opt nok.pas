...
uses
  ...
  uSymTab, uSymTabImpl, AnyObject, fgl, genericlists;
...
type
  ...
  TSelectExecutor = class(TStatementExecutor)
    private
      type
        TJumpTable = specialize TDictionary<TObject, ICodeNode>;
        TJumpCache = specialize TDictionary<ICodeNode, TJumpTable>;

      function CreateJumpTable(Node: ICodeNode): TJumpTable;
      class var JumpCache: TJumpCache;
    public
...
function TSelectExecutor.CreateJumpTable(Node: ICodeNode): TJumpTable;
var
  JumpTable: TJumpTable;
  branchNode, constantsNode, statementNode, constNode: ICodeNode;
  selectChildren, constantsList: TICodeNodeList;
  Value: TObject;
  i: integer;
begin
  ...
  for i := 1 to selectChildren.Count - 1 do begin
    branchNode := selectChildren[i];
    constantsNode := branchNode.getChildren[0];
    statementNode := branchNode.getChildren[1];
    ...
    for constNode in constantsList do begin
      ...
      Value := constNode.getAttribute(ckValue);
...
class constructor TSelectExecutor.Create;
begin
  JumpCache := TJumpCache.Create;
end;
...
var
  JumpTable: TJumpTable = Nil;
  selectChildren: TICodeNodeList;
  exprNode, statementNode: ICodeNode;
  Index: integer = -1;
  ExpressionExecutor: TExpressionExecutor;
  StatementExecutor: TStatementExecutor;
  selectValue: TObject;
begin
  ...
  if JumpCache.Find(Node, Index) then
    JumpTable := JumpCache.Data[Index];
  if JumpTable = Nil then begin
  ...
  if JumpTable.Find(selectValue, Index) then
    statementNode := JumpTable.Data[Index]
  else
    statementNode := Nil;
