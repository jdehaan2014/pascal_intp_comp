...
uses
  ...
  uTypesImpl, uPredefined, uSymTabImpl, fgl, anyobject, uIDEControl;
...
//listener for back end messages
procedure TGUIDebugger.TBackendMessageListener.MessageReceived(Message: TMessage);
begin
  ProcessMessage(Message);
end;


constructor TGUIDebugger.Create
  (Backend: TBackend; RuntimeStack: IRuntimeStack);
begin
  inherited Create(Backend, RuntimeStack);
  CommandProcessor := TCommandProcessor.Create(Self);
...
//process a msg from the backend
class procedure TGUIDebugger.ProcessMessage(Message: TMessage);
begin
  CommandProcessor.ProcessMessage(Message);
end;

//Display a prompt for a debugger command
procedure TGUIDebugger.PromptForCommand;
begin
  //nothing yet
end;

//Parse a debugger command
function TGUIDebugger.ParseCommand: boolean;
begin
  Result := CommandProcessor.ParseCommand;
end;

//process a source statement
procedure TGUIDebugger.atStatement(LineNumber: Integer);
begin
  WriteLn(DEBUGGER_AT_TAG, LineNumber);
end;

//process a breakpoint at a statement
procedure TGUIDebugger.atBreakpoint(LineNumber: Integer);
begin
  WriteLn(DEBUGGER_BREAK_TAG, LineNumber);
end;

//process the current value of a watchpoint variablet
procedure TGUIDebugger.atWatchpointValue
  (LineNumber: Integer; Name: String; Value: TObject);
begin
  //nothing yet
end;
...
//process the assigning a new value to a watchpoint variablet
procedure TGUIDebugger.atWatchPointAssignment
  (LineNumber: Integer; Name: String; Value: TObject);
begin
  //nothing yet
end;
...
//process calling a declared procedure or function
procedure TGUIDebugger.CallRoutine
  (LineNumber: Integer; RoutineName: String);
begin
  //nothing yet
end;
...
//process returning from a declared procedure or function
procedure TGUIDebugger.ReturnRoutine
  (LineNumber: Integer; RoutineName: String);
begin
  //nothing yet
end;
...
//Display a value
procedure TGUIDebugger.DisplayValue(ValueString: TString);
begin
  Writeln(ValueString.Value);
end;

//Display the call stack
procedure TGUIDebugger.DisplayCallStack(Stack: TObjectStack);
var
  Item: TObject;
  RoutineID: ISymTabEntry;
  RoutineName: string;
...
begin
  //call stack header
  writeln(DEBUGGER_ROUTINE_TAG, -1);
  for Item in Stack do begin
    //name of procedure or function
    if Item is ISymTabEntry then begin
      RoutineID := Item as ISymTabEntry;
      ...
      Level := RoutineID.getSymTab.getNestingLevel;
      Definition := RoutineID.getDefinition;
      WriteLn(DEBUGGER_ROUTINE_TAG, Level, ': ', Definition.getText.toUpper, ' ',
        RoutineName);
    end
    else if Item is TNameValuePair then begin // Variable name-value pair
      Pair := TNameValuePair(Item);
      Write(DEBUGGER_VARIABLE_TAG, Pair.getVariableName, ': ');
      DisplayValue(Pair.getValueString);
    end;
  end;
  //call stack footer
  writeln(DEBUGGER_ROUTINE_TAG, -2);
end;

//Terminate execution of the source program
procedure TGUIDebugger.Quit;
begin
  WriteLn('!INTERPRETER:Program terminated.');
  Halt(-1);
end;

//Handle a debugger command error
procedure TGUIDebugger.CommandError(ErrorMessage: string);
begin
  RuntimeError(ErrorMessage, 0);
end;

//Handle a source program RUNTIME error
procedure TGUIDebugger.RuntimeError
  (ErrorMessage: string; LineNumber: Integer);
begin
  Write('*** RUNTIME ERROR');
  if LineNumber > 0 then
    Write(' AT LINE ', Format('%.3d', [LineNumber]));
  WriteLn(': ',  ErrorMessage);
end;
